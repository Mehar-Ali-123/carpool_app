...
const OfferRide = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const finalData = {...data, rider: userJson._id };
    try {
      const res = await axios.post(`${config.api}/offers/add`, finalData);
      const result = res.data;
      if(result.success){
        Alert.alert('Success', "Offer has been sent!", [{text: "ok"}])
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity onPress={() =>  navigation.navigate('DriverRoots')}><Text>Passender Roots</Text></TouchableOpacity>
